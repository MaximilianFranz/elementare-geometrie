...
\textbf{Behauptung:} 
\textbf{Beweis:} \\
\emph{Erinnerung:} Sing $ x, x': U \to \mathbb{R}^3 $ param. Flächenstücke und ist $I_x(u,v) = I_{ x' }(u,v)$ für alle $ (u,v) \in U $, dann sind $ x(U), x'(U) $ lokal isometrisch durch $ x' \circ x^{ -1 }$\\
\emph{Anwendung:} Berechne: \\
\begin{align*}
	x_u = (\begin{smallmatrix}	
	-sin(u)\\
	cos(u)\\
	0	
	\end{smallmatrix}),
	x_v= (\begin{smallmatrix}	
	0\\
	0\\
	1	
	\end{smallmatrix}	) \\
		x'_u = (\begin{smallmatrix}	
	1\\
	0\\
	0	
	\end{smallmatrix}),
	x'_v= (\begin{smallmatrix}	
	0\\
	1\\
	0	
	\end{smallmatrix}	)
...
\begin{pmatrix}
	...
	\begin{pmatrix}
		...
		-\varphi(v) \varphi'(v)
	\end{pmatrix},
	\Vert x_u x x_v \Vert = \sqrt{ \varphi(v)^2 \psi'(v)^2 + \varphi(v)^2 \varphi'(v)^2} \\
...
\end{align*}
\emph{Anmerkung:} Hier bin ich mir mit den Variable von $\varphi, \psi$ nicht ganz sicher.. Für Verbesserungen gerne melden.
\end{problem*}

\begin{problem*}[2b]
Geben Sie Beispiele von Rotationsflächen mit konstanter Gauß-Krümmung 1, 0, -1 an.\\
Wir suche nfür ein vorgegebens $ K $ solche $\varphi, \psi$ sodass (\*) erfüllt wird. Um unsere Suche einzuschränken, können wir Bedingungen an $\varphi, \psi$ stellen. \\s
...
\end{equation*}	
\emph{Zitat:} "Das ist nicht so schön, es ist sogar ziemlich hässlich." \\
...
Nun ist 
\begin{equation*}
	K = \frac{\varphi' \psi' \psi'' - \psi'^2 \varphi''}{\varphi\underset{=1}{ (\varphi'^2 + \psi'^2) }^2} = \frac{-\varphi'^2 \varphi'' - \psi'^2 \varphi''}{\varphi} = \frac{-\phi''}{\varphi}
\end{equation*}
Damit ist $\varphi'' = -K \varphi$ (**)
\textbf{Fall K = 1}: Die Lösungen von $\varphi'' = - \varphi$ sind $\varphi(v) = a cosv + b sinv (a,b \in \mathbb{R})$. \\
...
%----------------------------------------------------------------------------------------
\textbf{Fall K = 0:} Aus (**) folgt $ \varphi'' = 0 $ das heißt: \\
...
&= \tilde{ c }''(t) = 0\tilde{ c }'(t) + \kappa_g(t)(\tilde{ n }(t) \land \tilde{c}(l-t) + \tilde{\alpha}(t) \tilde{n}(t)) \\
&= -0 c'(l-t) - \tilde{\kappa_g}(t)(n(l-t) \land c'(l-t)) + \tilde{ \alpha }(t) \cdot n(l-t)
\end{align*}
Durch Koeffizientenvergleich: \\
$-0 = 0, \tilde{ \alpha }(t) = \alpha(l-t)$. Daraus folgt direkt die Behauptung.
