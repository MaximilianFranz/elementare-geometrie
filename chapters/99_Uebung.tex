...
\section{2018-01-26 - Probeklausur} % (fold)
\label{sec:2018_01_26_probeklausur}
\emph{Hinweis:} Die Lösungen der Probeklausur sind auf der Vorlesungsseite zu finden.

% section 2018_01_26_probeklausur (end)

\section{2018-02-02 - Übungsblatt 11} % (fold)
\label{sec:übungsblatt_11}
\begin{problem*}[1]
\textbf{Vorraussetzung:} Parametrisierung des Rotationstorus!\\
\textbf{Zu zeigen:} Zeige, wo auf dem Torus die Gauß-Krümmung $ K $ größer 0, kleiner 0 oder verschwindend ist.\\s
\emph{Erster Weg:} Gauß-Krümmung ausrechnen und entsprechend auflösen... Rechenaufwand!\\
...
\textbf{Beweis:} Sei $Ü$ eine offene Überdeckung von $\bar{\mathbb{C}} $.\\
...
Die Menge $U \coloneqq \{ \tilde{ Ü } \setminus U_{ \infty } : U \in Ü \}$ ist offene Überdeckung von $\bar{\mathbb{C}} \setminus U_{ \infty }$, denn für alle $ x \in \bar{\mathbb{C}} \setminus U_{ \infty } $ gibt es ein $U_x \in Ü$ mit $x \in U_x$, also auch $\tilde{ Ü_x } \coloneqq U_x \setminus U_{ \infty } \in \tilde{ Ü }$ mit $x \in \tilde{ U_x }.$\\
Da $\tilde{ Ü }$ offene Überdeckung einer kompakten Menge ist hat $\tilde{ Ü }$ eine endliche Teilüberdeckung: 
\begin{equation*}
  \{ U_1 \setminus U_{ \infty }, \dots, U_n \setminus U_{ \infty } \}, U_1, \dots, U_n \in Ü
\end{equation*}
Dann ist $\{ U_1, \dots, U_n, U_{ \infty } \}$ eine endliche Teilüberdeckung von $ Ü $, denn für alle
...
\emph{Beweis des Lemma:} \\s
Sei $A = \begin{smallmatrix}
a & b \\
c & d
\end{smallmatrix}, B = \begin{smallmatrix}
e & f \\
g & h
\end{smallmatrix}$. \\
% ---------------------------------------------------------------------------------------
...
= \frac{a(ez+f)+b(gz+h)}{c(ez+f)+d(gz+h)} = \frac{(ae+bg)z + (af+bh)}{(ce+dg)z + (cf +dh)}= M_{ \begin{smallmatrix}
ae+bg & af+bh \\
ce+dg & cf+dh 
\end{smallmatrix} }
\end{align*}
...
\begin{align*}
M_A(M_B(z)) = M_A(\frac{ez+f}{gz+h}) = M_A(\infty) = \frac{a}{c} = \frac{a(ez+f)}{c(ez+f)} \\
= \frac{a(ez+f) + b(gz+h)}{c(ez+f)+d(gz+h)} = M_{ AB }(z)
\end{align*}
\underline{Fall 3:}Für $ z = \infty, g \neq 0$: 
\begin{align*}
M_A(M_B(\infty)) = M_A(\frac{e}{g}) = \frac{a \cdot \frac{e}{g} + b}{c \cdot \frac{e}{g} + d}
 = \frac{ae + bg}{ce+dg} = M_{ AB }(\infty)  
\end{align*}
\underline{Fall 4}: $ z = \infty, g = 0:$
\begin{align*}
M_A(M_B(\infty)) = M_A(\frac{e}{g}) = M_A(\infty) = \frac{a}{c} = \frac{ae}{ce} = \frac{ae + bg}{ce + dg} = M_{ AB }(\infty) 
\end{align*}
\textbf{Beweis Aufgabe:} Sei nun $A \in GL(2,\mathbb{C})$ beliebig. Nach Lemma ist:
\begin{equation*}
  M_A \circ M_{ A^{ -1 } } = M_{ \begin{smallmatrix}
  1 & 0 \\
  0 & 1 
  \end{smallmatrix} } = M_{ A^{-1 } } \circ M_A
\end{equation*}
und 
\begin{equation*}
...
Damit ist
\begin{equation*}
z \in M(A_1) \cup M(A_2) \cup M(A_3) = M(A_1 \cup A_2 \cup A_3) = M(\bar{\mathbb{C}}) = \bar{\mathbb{C}} \lightning
\end{equation*}
Damit ist dann:
\begin{itemize}
