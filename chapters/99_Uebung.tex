...
\subsection{2017-12-01 - Übungsblatt 6} % 
\label{2017-12-01}

\textbf{Muss nochmal gesäubert werden!!!}
...
\textbf{Beh:} Es gibt $N \in \mathbb{N}, P, Q \in \mathbb{R}[x,y]$ sodass:
\begin{equation*}
...
\textbf{Beweis:}
\begin{equation*}
  \frac{d}{dx} g(x,y) = \left((x^2 + y^2) p_x(x,y) - 2x p(x,y), (x^2 + y^2) q_x(x,y) - 2x q(x,y)\right)
...
\begin{problem*}[2a]
Der Tangentialraum $ T_pF$ von $ F $ in $ p $ sei definiert als\\
 $ T_pF \coloneqq Bild(d \phi (\varphi^{ -1 }(p))) $, wobei $\varphi: V \to F, V \subseteq \mathbb{ R }^2$,\\

 eine Parametrisierung von $ F $ um den Punkt $ p \in F$ ist.
...
\textbf{Beweis:} Seien $ \varphi, \psi$ Param. von $ F $ um $ p $. Sei $q=\varphi^{ -1 }(p), r=\psi^{ -1 }(p).$\\
zz: $Bild(d \phi(q)) = Bild(d \psi(r))$.\\
