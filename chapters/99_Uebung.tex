...
Für alle $ s \in S$ gibt es mindestens $ n $ Kanten $k \in K$, sodass $ (k,s) \in W $, denn falls $ s $ beschränkt ist, der äußere Rand von $ s $ ebsteht aus einem Kreis, der min $ n $ Kanten hat. Wenn $ s $ unbeschränkt:
\begin{itemize}
...
\begin{problem*}[1b]
Zeichnungen müssen eingefügt werden. !
% ---------------------------------------------------------------------------------------
% Zeichnungen zu Aufgabe A1 b) einfügen!
%----------------------------------------------------------------------------------------
\end{problem*}
...
\begin{align*}
   ...
   Sym(P) &\coloneqq \{ f \in SO(3) : f(P) = P \}
 \end{align*} 
Sei $ P $ Würfel. \\
\textbf{Zu zeigen:} 
\begin{itemize}
...
Damit ist $ \vert Sym(P) \leq 6 \cdot 8 = 48 \vert $. \\
$Sym_0(P) $ ist Untergruppe von $ Sym(P) $. Nach dem Satz von Lagrage ist $\frac{\vert Sym(P) \vert}{\vert Sym_0(P) \vert} \in \mathbb{N} (*)$.\\
Da $\begin{pmatrix}
	-1 & 0 & 0 \\
	0 & -1 & 0 \\
	0 & 0 & -1 
\end{pmatrix} \in Sym(P)$, aber $det(-I) = (-1)^3 = -1$, ist $I \in Sym(P) \setminus Sym_0(P)$, also ist $\vert Sym(P) \vert > \vert Sym_0(P) \vert $.\\
Wegen $ (*) $ folgt
\begin{equation*}
...
Wir haben einen sujektiven Hom. $h: Sym_0(P) \to B_{ ij }(D)$. Aus $ \vert Sym_0(P) \vert \leq 24 = \vert S_4 \vert = \vert B_{ ij }(D) \vert$ folgt sofort, dass $\vert Sym_0(P) \vert = \vert B_{ ij }(D) \vert$ ist und dass $ h $ bijektiv ist:
\begin{equation*}
	Sym_0(P) \cong B_{ ij }(D) \cong S_4.
\end{equation*}
\textbf{Behauptung:} $Sym(P) \cong Sym_0(P) \times \{ -1, 1 \}.$\\
\textbf{Beweis:} $g: Sym(P) \to Sym_0(P) \times \{ -1, 1 \}, \psi \mapsto (det(\psi) \cdot \psi,det(\psi))$\\
\begin{equation*}
